/**
 * FHIR R4 Procedure Resource Definition
 * An action that is or was performed on or for a patient as part of healthcare provision.
...
export class FHIRProcedureValidator {
  static validate(resource: any): { isValid: boolean; errors: string[] } {
    ...
    if (resource.resourceType !== 'Procedure') {
      errors.push(`Invalid resourceType: expected 'Procedure', got '${resource.resourceType}'`);
    }
  ...
  static create(initial: Partial<FHIRProcedure>): FHIRProcedure {
    return {
      resourceType: 'Procedure',
  ...
  static toJSON(resource: FHIRProcedure): string {
    return JSON.stringify(resource, null, 2);
  }

  static fromJSON(jsonStr: string): FHIRProcedure {
    const parsed = JSON.parse(jsonStr);
    const validation = this.validate(parsed);
    if (!validation.isValid) {
      throw new Error(`FHIR Validation Error in Procedure: ` + validation.errors.join(', '));
    }
    return parsed as FHIRProcedure;
  }
}
