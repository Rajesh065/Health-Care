/**
 * FHIR R4 Organization Resource Definition
...
export class FHIROrganizationValidator {
  static validate(resource: any): { isValid: boolean; errors: string[] } {
    ...
    if (resource.resourceType !== 'Organization') {
      errors.push(`Invalid resourceType: expected 'Organization', got '${resource.resourceType}'`);
    }
  ...
  static create(initial: Partial<FHIROrganization>): FHIROrganization {
    return {
      resourceType: 'Organization',
  ...
  static toJSON(resource: FHIROrganization): string {
    return JSON.stringify(resource, null, 2);
  }

  static fromJSON(jsonStr: string): FHIROrganization {
    const parsed = JSON.parse(jsonStr);
    const validation = this.validate(parsed);
    if (!validation.isValid) {
      throw new Error(`FHIR Validation Error in Organization: ` + validation.errors.join(', '));
    }
    return parsed as FHIROrganization;
  }
}
