/**
 * FHIR R4 CareTeam Resource Definition
...
export class FHIRCareTeamValidator {
  static validate(resource: any): { isValid: boolean; errors: string[] } {
    ...
    if (resource.resourceType !== 'CareTeam') {
      errors.push(`Invalid resourceType: expected 'CareTeam', got '${resource.resourceType}'`);
    }
  ...
  static create(initial: Partial<FHIRCareTeam>): FHIRCareTeam {
    return {
      resourceType: 'CareTeam',
  ...
  static toJSON(resource: FHIRCareTeam): string {
    return JSON.stringify(resource, null, 2);
  }

  static fromJSON(jsonStr: string): FHIRCareTeam {
    const parsed = JSON.parse(jsonStr);
    const validation = this.validate(parsed);
    if (!validation.isValid) {
      throw new Error(`FHIR Validation Error in CareTeam: ` + validation.errors.join(', '));
    }
    return parsed as FHIRCareTeam;
  }
}
