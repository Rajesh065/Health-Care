/**
 * FHIR R4 Questionnaire Resource Definition
...
export class FHIRQuestionnaireValidator {
  ...
  static create(initial: Partial<FHIRQuestionnaire>): FHIRQuestionnaire {
    return {
      resourceType: 'Questionnaire',
  ...
  static toJSON(resource: FHIRQuestionnaire): string {
    return JSON.stringify(resource, null, 2);
  }

  static fromJSON(jsonStr: string): FHIRQuestionnaire {
    const parsed = JSON.parse(jsonStr);
    const validation = this.validate(parsed);
    if (!validation.isValid) {
      throw new Error(`FHIR Validation Error in Questionnaire: ` + validation.errors.join(', '));
    }
    return parsed as FHIRQuestionnaire;
  }
}
