/**
 * FHIR R4 Patient Resource Definition
 * Demographics and administrative information for an individual receiving healthcare services.
...
export class FHIRPatientValidator {
  static validate(resource: any): { isValid: boolean; errors: string[] } {
    ...
    if (resource.resourceType !== 'Patient') {
      errors.push(`Invalid resourceType: expected 'Patient', got '${resource.resourceType}'`);
    }
  ...
  static create(initial: Partial<FHIRPatient>): FHIRPatient {
    return {
      resourceType: 'Patient',
  ...
  static toJSON(resource: FHIRPatient): string {
    return JSON.stringify(resource, null, 2);
  }

  static fromJSON(jsonStr: string): FHIRPatient {
    const parsed = JSON.parse(jsonStr);
    const validation = this.validate(parsed);
    if (!validation.isValid) {
      throw new Error(`FHIR Validation Error in Patient: ` + validation.errors.join(', '));
    }
    return parsed as FHIRPatient;
  }
}
