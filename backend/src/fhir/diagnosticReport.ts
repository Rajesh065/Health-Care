/**
 * FHIR R4 DiagnosticReport Resource Definition
...
export class FHIRDiagnosticReportValidator {
  static validate(resource: any): { isValid: boolean; errors: string[] } {
    ...
    if (resource.resourceType !== 'DiagnosticReport') {
      errors.push(`Invalid resourceType: expected 'DiagnosticReport', got '${resource.resourceType}'`);
    }
  ...
  static create(initial: Partial<FHIRDiagnosticReport>): FHIRDiagnosticReport {
    return {
      resourceType: 'DiagnosticReport',
  ...
  static toJSON(resource: FHIRDiagnosticReport): string {
    return JSON.stringify(resource, null, 2);
  }

  static fromJSON(jsonStr: string): FHIRDiagnosticReport {
    const parsed = JSON.parse(jsonStr);
    const validation = this.validate(parsed);
    if (!validation.isValid) {
      throw new Error(`FHIR Validation Error in DiagnosticReport: ` + validation.errors.join(', '));
    }
    return parsed as FHIRDiagnosticReport;
  }
}
