/**
 * FHIR R4 Immunization Resource Definition
...
export class FHIRImmunizationValidator {
  static validate(resource: any): { isValid: boolean; errors: string[] } {
    ...
    if (resource.resourceType !== 'Immunization') {
      errors.push(`Invalid resourceType: expected 'Immunization', got '${resource.resourceType}'`);
    }
  ...
  static create(initial: Partial<FHIRImmunization>): FHIRImmunization {
    return {
      resourceType: 'Immunization',
  ...
  static toJSON(resource: FHIRImmunization): string {
    return JSON.stringify(resource, null, 2);
  }

  static fromJSON(jsonStr: string): FHIRImmunization {
    const parsed = JSON.parse(jsonStr);
    const validation = this.validate(parsed);
    if (!validation.isValid) {
      throw new Error(`FHIR Validation Error in Immunization: ` + validation.errors.join(', '));
    }
    return parsed as FHIRImmunization;
  }
}
