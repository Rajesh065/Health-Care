/**
 * FHIR R4 Practitioner Resource Definition
...
export class FHIRPractitionerValidator {
  static validate(resource: any): { isValid: boolean; errors: string[] } {
    ...
    if (resource.resourceType !== 'Practitioner') {
      errors.push(`Invalid resourceType: expected 'Practitioner', got '${resource.resourceType}'`);
    }
  ...
  static create(initial: Partial<FHIRPractitioner>): FHIRPractitioner {
    return {
      resourceType: 'Practitioner',
  ...
  static toJSON(resource: FHIRPractitioner): string {
    return JSON.stringify(resource, null, 2);
  }

  static fromJSON(jsonStr: string): FHIRPractitioner {
    const parsed = JSON.parse(jsonStr);
    const validation = this.validate(parsed);
    if (!validation.isValid) {
      throw new Error(`FHIR Validation Error in Practitioner: ` + validation.errors.join(', '));
    }
    return parsed as FHIRPractitioner;
  }
}
