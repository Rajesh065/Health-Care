/**
 * FHIR R4 Device Resource Definition
 * A type of a manufactured item that is used in the provision of healthcare without being substantially changed through that use.
...
export class FHIRDeviceValidator {
  static validate(resource: any): { isValid: boolean; errors: string[] } {
    ...
    if (resource.resourceType !== 'Device') {
      errors.push(`Invalid resourceType: expected 'Device', got '${resource.resourceType}'`);
    }
  ...
  static create(initial: Partial<FHIRDevice>): FHIRDevice {
    return {
      resourceType: 'Device',
  ...
  static toJSON(resource: FHIRDevice): string {
    return JSON.stringify(resource, null, 2);
  }

  static fromJSON(jsonStr: string): FHIRDevice {
    const parsed = JSON.parse(jsonStr);
    const validation = this.validate(parsed);
    if (!validation.isValid) {
      throw new Error(`FHIR Validation Error in Device: ` + validation.errors.join(', '));
    }
    return parsed as FHIRDevice;
  }
}
