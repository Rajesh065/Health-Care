/**
 * FHIR R4 QuestionnaireResponse Resource Definition
...
export class FHIRQuestionnaireResponseValidator {
  static validate(resource: any): { isValid: boolean; errors: string[] } {
    ...
    if (resource.resourceType !== 'QuestionnaireResponse') {
      errors.push(`Invalid resourceType: expected 'QuestionnaireResponse', got '${resource.resourceType}'`);
    }
  ...
  static create(initial: Partial<FHIRQuestionnaireResponse>): FHIRQuestionnaireResponse {
    return {
      resourceType: 'QuestionnaireResponse',
  ...
  static toJSON(resource: FHIRQuestionnaireResponse): string {
    return JSON.stringify(resource, null, 2);
  }

  static fromJSON(jsonStr: string): FHIRQuestionnaireResponse {
    const parsed = JSON.parse(jsonStr);
    const validation = this.validate(parsed);
    if (!validation.isValid) {
      throw new Error(`FHIR Validation Error in QuestionnaireResponse: ` + validation.errors.join(', '));
    }
    return parsed as FHIRQuestionnaireResponse;
  }
}
