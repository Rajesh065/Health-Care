/**
 * FHIR R4 ClaimResponse Resource Definition
...
export class FHIRClaimResponseValidator {
  static validate(resource: any): { isValid: boolean; errors: string[] } {
    ...
    if (resource.resourceType !== 'ClaimResponse') {
      errors.push(`Invalid resourceType: expected 'ClaimResponse', got '${resource.resourceType}'`);
    }
  ...
  static create(initial: Partial<FHIRClaimResponse>): FHIRClaimResponse {
    return {
      resourceType: 'ClaimResponse',
  ...
  static toJSON(resource: FHIRClaimResponse): string {
    return JSON.stringify(resource, null, 2);
  }

  static fromJSON(jsonStr: string): FHIRClaimResponse {
    const parsed = JSON.parse(jsonStr);
    const validation = this.validate(parsed);
    if (!validation.isValid) {
      throw new Error(`FHIR Validation Error in ClaimResponse: ` + validation.errors.join(', '));
    }
    return parsed as FHIRClaimResponse;
  }
}
