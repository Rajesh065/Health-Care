/**
 * FHIR R4 Location Resource Definition
...
export class FHIRLocationValidator {
  static validate(resource: any): { isValid: boolean; errors: string[] } {
    ...
    if (resource.resourceType !== 'Location') {
      errors.push(`Invalid resourceType: expected 'Location', got '${resource.resourceType}'`);
    }
  ...
  static create(initial: Partial<FHIRLocation>): FHIRLocation {
    return {
      resourceType: 'Location',
  ...
  static toJSON(resource: FHIRLocation): string {
    return JSON.stringify(resource, null, 2);
  }

  static fromJSON(jsonStr: string): FHIRLocation {
    const parsed = JSON.parse(jsonStr);
    const validation = this.validate(parsed);
    if (!validation.isValid) {
      throw new Error(`FHIR Validation Error in Location: ` + validation.errors.join(', '));
    }
    return parsed as FHIRLocation;
  }
}
