/**
 * FHIR R4 Slot Resource Definition
...
export class FHIRSlotValidator {
  static validate(resource: any): { isValid: boolean; errors: string[] } {
    ...
    if (resource.resourceType !== 'Slot') {
      errors.push(`Invalid resourceType: expected 'Slot', got '${resource.resourceType}'`);
    }
  ...
  static create(initial: Partial<FHIRSlot>): FHIRSlot {
    return {
      resourceType: 'Slot',
  ...
  static toJSON(resource: FHIRSlot): string {
    return JSON.stringify(resource, null, 2);
  }

  static fromJSON(jsonStr: string): FHIRSlot {
    const parsed = JSON.parse(jsonStr);
    const validation = this.validate(parsed);
    if (!validation.isValid) {
      throw new Error(`FHIR Validation Error in Slot: ` + validation.errors.join(', '));
    }
    return parsed as FHIRSlot;
  }
}
