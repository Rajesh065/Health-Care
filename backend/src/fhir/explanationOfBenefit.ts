/**
 * FHIR R4 ExplanationOfBenefit Resource Definition
 * This resource provides: the adjudication details for a Claim, the total amounts and balance, and the payment details.
...
export class FHIRExplanationOfBenefitValidator {
  static validate(resource: any): { isValid: boolean; errors: string[] } {
    ...
    if (resource.resourceType !== 'ExplanationOfBenefit') {
      errors.push(`Invalid resourceType: expected 'ExplanationOfBenefit', got '${resource.resourceType}'`);
    }
  ...
  static create(initial: Partial<FHIRExplanationOfBenefit>): FHIRExplanationOfBenefit {
    return {
      resourceType: 'ExplanationOfBenefit',
  ...
  static toJSON(resource: FHIRExplanationOfBenefit): string {
    return JSON.stringify(resource, null, 2);
  }

  static fromJSON(jsonStr: string): FHIRExplanationOfBenefit {
    const parsed = JSON.parse(jsonStr);
    const validation = this.validate(parsed);
    if (!validation.isValid) {
      throw new Error(`FHIR Validation Error in ExplanationOfBenefit: ` + validation.errors.join(', '));
    }
    return parsed as FHIRExplanationOfBenefit;
  }
}
