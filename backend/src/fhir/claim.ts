/**
 * FHIR R4 Claim Resource Definition
 * A provider issued list of professional services and products provided, or to be provided, to a patient which is submitted to an insurer for payment.
...
export class FHIRClaimValidator {
  static validate(resource: any): { isValid: boolean; errors: string[] } {
    ...
    if (resource.resourceType !== 'Claim') {
      errors.push(`Invalid resourceType: expected 'Claim', got '${resource.resourceType}'`);
    }
  ...
  static create(initial: Partial<FHIRClaim>): FHIRClaim {
    return {
      resourceType: 'Claim',
  ...
  static toJSON(resource: FHIRClaim): string {
    return JSON.stringify(resource, null, 2);
  }

  static fromJSON(jsonStr: string): FHIRClaim {
    const parsed = JSON.parse(jsonStr);
    const validation = this.validate(parsed);
    if (!validation.isValid) {
      throw new Error(`FHIR Validation Error in Claim: ` + validation.errors.join(', '));
    }
    return parsed as FHIRClaim;
  }
}
