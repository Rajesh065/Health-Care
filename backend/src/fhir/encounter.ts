/**
 * FHIR R4 Encounter Resource Definition
 * An interaction between a patient and healthcare provider for the purpose of providing healthcare services.
...
export class FHIREncounterValidator {
  static validate(resource: any): { isValid: boolean; errors: string[] } {
    ...
    if (resource.resourceType !== 'Encounter') {
      errors.push(`Invalid resourceType: expected 'Encounter', got '${resource.resourceType}'`);
    }
  ...
  static create(initial: Partial<FHIREncounter>): FHIREncounter {
    return {
      resourceType: 'Encounter',
  ...
  static toJSON(resource: FHIREncounter): string {
    return JSON.stringify(resource, null, 2);
  }

  static fromJSON(jsonStr: string): FHIREncounter {
    const parsed = JSON.parse(jsonStr);
    const validation = this.validate(parsed);
    if (!validation.isValid) {
      throw new Error(`FHIR Validation Error in Encounter: ` + validation.errors.join(', '));
    }
    return parsed as FHIREncounter;
  }
}
