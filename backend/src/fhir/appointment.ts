/**
 * FHIR R4 Appointment Resource Definition
 * A booking of a healthcare event among patient, practitioner, related person and/or device for a specific date/time.
...
export class FHIRAppointmentValidator {
  static validate(resource: any): { isValid: boolean; errors: string[] } {
    ...
    if (resource.resourceType !== 'Appointment') {
      errors.push(`Invalid resourceType: expected 'Appointment', got '${resource.resourceType}'`);
    }
  ...
  static create(initial: Partial<FHIRAppointment>): FHIRAppointment {
    return {
      resourceType: 'Appointment',
  ...
  static toJSON(resource: FHIRAppointment): string {
    return JSON.stringify(resource, null, 2);
  }

  static fromJSON(jsonStr: string): FHIRAppointment {
    const parsed = JSON.parse(jsonStr);
    const validation = this.validate(parsed);
    if (!validation.isValid) {
      throw new Error(`FHIR Validation Error in Appointment: ` + validation.errors.join(', '));
    }
    return parsed as FHIRAppointment;
  }
}
