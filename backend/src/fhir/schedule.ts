/**
 * FHIR R4 Schedule Resource Definition
...
export class FHIRScheduleValidator {
  static validate(resource: any): { isValid: boolean; errors: string[] } {
    ...
    if (resource.resourceType !== 'Schedule') {
      errors.push(`Invalid resourceType: expected 'Schedule', got '${resource.resourceType}'`);
    }
  ...
  static create(initial: Partial<FHIRSchedule>): FHIRSchedule {
    return {
      resourceType: 'Schedule',
  ...
  static toJSON(resource: FHIRSchedule): string {
    return JSON.stringify(resource, null, 2);
  }

  static fromJSON(jsonStr: string): FHIRSchedule {
    const parsed = JSON.parse(jsonStr);
    const validation = this.validate(parsed);
    if (!validation.isValid) {
      throw new Error(`FHIR Validation Error in Schedule: ` + validation.errors.join(', '));
    }
    return parsed as FHIRSchedule;
  }
}
