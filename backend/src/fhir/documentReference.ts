/**
 * FHIR R4 DocumentReference Resource Definition
...
export class FHIRDocumentReferenceValidator {
  static validate(resource: any): { isValid: boolean; errors: string[] } {
    ...
    if (resource.resourceType !== 'DocumentReference') {
      errors.push(`Invalid resourceType: expected 'DocumentReference', got '${resource.resourceType}'`);
    }
  ...
  static create(initial: Partial<FHIRDocumentReference>): FHIRDocumentReference {
    return {
      resourceType: 'DocumentReference',
  ...
  static toJSON(resource: FHIRDocumentReference): string {
    return JSON.stringify(resource, null, 2);
  }

  static fromJSON(jsonStr: string): FHIRDocumentReference {
    const parsed = JSON.parse(jsonStr);
    const validation = this.validate(parsed);
    if (!validation.isValid) {
      throw new Error(`FHIR Validation Error in DocumentReference: ` + validation.errors.join(', '));
    }
    return parsed as FHIRDocumentReference;
  }
}
