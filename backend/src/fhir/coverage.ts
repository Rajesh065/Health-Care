/**
 * FHIR R4 Coverage Resource Definition
 * Financial instrument which may be used to pay for or reimburse healthcare products and services.
...
export class FHIRCoverageValidator {
  static validate(resource: any): { isValid: boolean; errors: string[] } {
    ...
    if (resource.resourceType !== 'Coverage') {
      errors.push(`Invalid resourceType: expected 'Coverage', got '${resource.resourceType}'`);
    }
  ...
  static create(initial: Partial<FHIRCoverage>): FHIRCoverage {
    return {
      resourceType: 'Coverage',
  ...
  static toJSON(resource: FHIRCoverage): string {
    return JSON.stringify(resource, null, 2);
  }

  static fromJSON(jsonStr: string): FHIRCoverage {
    const parsed = JSON.parse(jsonStr);
    const validation = this.validate(parsed);
    if (!validation.isValid) {
      throw new Error(`FHIR Validation Error in Coverage: ` + validation.errors.join(', '));
    }
    return parsed as FHIRCoverage;
  }
}
