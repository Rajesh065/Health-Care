/**
 * FHIR R4 ServiceRequest Resource Definition
...
export class FHIRServiceRequestValidator {
  static validate(resource: any): { isValid: boolean; errors: string[] } {
    ...
    if (resource.resourceType !== 'ServiceRequest') {
      errors.push(`Invalid resourceType: expected 'ServiceRequest', got '${resource.resourceType}'`);
    }
  ...
  static create(initial: Partial<FHIRServiceRequest>): FHIRServiceRequest {
    return {
      resourceType: 'ServiceRequest',
  ...
  static toJSON(resource: FHIRServiceRequest): string {
    return JSON.stringify(resource, null, 2);
  }

  static fromJSON(jsonStr: string): FHIRServiceRequest {
    const parsed = JSON.parse(jsonStr);
    const validation = this.validate(parsed);
    if (!validation.isValid) {
      throw new Error(`FHIR Validation Error in ServiceRequest: ` + validation.errors.join(', '));
    }
    return parsed as FHIRServiceRequest;
  }
}
