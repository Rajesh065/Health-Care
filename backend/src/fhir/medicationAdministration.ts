/**
 * FHIR R4 MedicationAdministration Resource Definition
...
export class FHIRMedicationAdministrationValidator {
  static validate(resource: any): { isValid: boolean; errors: string[] } {
    ...
    if (resource.resourceType !== 'MedicationAdministration') {
      errors.push(`Invalid resourceType: expected 'MedicationAdministration', got '${resource.resourceType}'`);
    }
  ...
  static create(initial: Partial<FHIRMedicationAdministration>): FHIRMedicationAdministration {
    return {
      resourceType: 'MedicationAdministration',
  ...
  static toJSON(resource: FHIRMedicationAdministration): string {
    return JSON.stringify(resource, null, 2);
  }

  static fromJSON(jsonStr: string): FHIRMedicationAdministration {
    const parsed = JSON.parse(jsonStr);
    const validation = this.validate(parsed);
    if (!validation.isValid) {
      throw new Error(`FHIR Validation Error in MedicationAdministration: ` + validation.errors.join(', '));
    }
    return parsed as FHIRMedicationAdministration;
  }
}
