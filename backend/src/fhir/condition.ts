/**
 * FHIR R4 Condition Resource Definition
...
export class FHIRConditionValidator {
  static validate(resource: any): { isValid: boolean; errors: string[] } {
    ...
    if (resource.resourceType !== 'Condition') {
      errors.push(`Invalid resourceType: expected 'Condition', got '${resource.resourceType}'`);
    }
  ...
  static create(initial: Partial<FHIRCondition>): FHIRCondition {
    return {
      resourceType: 'Condition',
  ...
  static toJSON(resource: FHIRCondition): string {
    return JSON.stringify(resource, null, 2);
  }

  static fromJSON(jsonStr: string): FHIRCondition {
    const parsed = JSON.parse(jsonStr);
    const validation = this.validate(parsed);
    if (!validation.isValid) {
      throw new Error(`FHIR Validation Error in Condition: ` + validation.errors.join(', '));
    }
    return parsed as FHIRCondition;
  }
}
