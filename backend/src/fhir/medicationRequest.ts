/**
 * FHIR R4 MedicationRequest Resource Definition
 * An order or request for both supply of the item to a patient and instructions for administration of the medication.
...
export class FHIRMedicationRequestValidator {
  static validate(resource: any): { isValid: boolean; errors: string[] } {
    ...
    if (resource.resourceType !== 'MedicationRequest') {
      errors.push(`Invalid resourceType: expected 'MedicationRequest', got '${resource.resourceType}'`);
    }
  ...
  static create(initial: Partial<FHIRMedicationRequest>): FHIRMedicationRequest {
    return {
      resourceType: 'MedicationRequest',
  ...
  static toJSON(resource: FHIRMedicationRequest): string {
    return JSON.stringify(resource, null, 2);
  }

  static fromJSON(jsonStr: string): FHIRMedicationRequest {
    const parsed = JSON.parse(jsonStr);
    const validation = this.validate(parsed);
    if (!validation.isValid) {
      throw new Error(`FHIR Validation Error in MedicationRequest: ` + validation.errors.join(', '));
    }
    return parsed as FHIRMedicationRequest;
  }
}
