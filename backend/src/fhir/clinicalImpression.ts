/**
 * FHIR R4 ClinicalImpression Resource Definition
 * A record of a clinical assessment performed to determine what problem(s) may affect the patient, and what diagnostic or therapeutic actions are needed.
...
export class FHIRClinicalImpressionValidator {
  static validate(resource: any): { isValid: boolean; errors: string[] } {
    ...
    if (resource.resourceType !== 'ClinicalImpression') {
      errors.push(`Invalid resourceType: expected 'ClinicalImpression', got '${resource.resourceType}'`);
    }
  ...
  static create(initial: Partial<FHIRClinicalImpression>): FHIRClinicalImpression {
    return {
      resourceType: 'ClinicalImpression',
  ...
  static toJSON(resource: FHIRClinicalImpression): string {
    return JSON.stringify(resource, null, 2);
  }

  static fromJSON(jsonStr: string): FHIRClinicalImpression {
    const parsed = JSON.parse(jsonStr);
    const validation = this.validate(parsed);
    if (!validation.isValid) {
      throw new Error(`FHIR Validation Error in ClinicalImpression: ` + validation.errors.join(', '));
    }
    return parsed as FHIRClinicalImpression;
  }
}
