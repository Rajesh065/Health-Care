/**
 * Clinical Calculator: Body Mass Index (BMI) and Mosteller Body Surface Area (BSA) Calculator
...
export interface BmiBsaCalculatorResult {
  score: number;
...
export const BmiBsaCalculator: React.FC = () => {
  ...
  const [result, setResult] = useState<BmiBsaCalculatorResult | null>(null);
  ...
  const calculate = (e: React.FormEvent) => {
    ...
    setResult({
      score: rawScore,
      riskCategory: cat,
      clinicalInterpretation: `Calculated clinical index indicates ${cat} risk profile based on validated General Medicine scoring criteria.`,
      recommendations: [
  ...
  return (
    <div className="bg-white rounded-2xl p-6 border border-slate-200 shadow-sm space-y-4">
      <div>
        <h3 className="text-base font-black text-slate-900">Body Mass Index (BMI) and Mosteller Body Surface Area (BSA) Calculator</h3>
        <p className="text-xs text-slate-500">Specialty Domain: General Medicine</p>
      </div>
