/**
 * Clinical Calculator: Child-Pugh Classification for Cirrhosis Mortality and Surgical Risk
...
export interface ChildPughCalculatorResult {
  score: number;
...
export const ChildPughCalculator: React.FC = () => {
  ...
  const [result, setResult] = useState<ChildPughCalculatorResult | null>(null);
  ...
  const calculate = (e: React.FormEvent) => {
    ...
    setResult({
      score: rawScore,
      riskCategory: cat,
      clinicalInterpretation: `Calculated clinical index indicates ${cat} risk profile based on validated Hepatology scoring criteria.`,
      recommendations: [
  ...
  return (
    <div className="bg-white rounded-2xl p-6 border border-slate-200 shadow-sm space-y-4">
      <div>
        <h3 className="text-base font-black text-slate-900">Child-Pugh Classification for Cirrhosis Mortality and Surgical Risk</h3>
        <p className="text-xs text-slate-500">Specialty Domain: Hepatology</p>
      </div>
