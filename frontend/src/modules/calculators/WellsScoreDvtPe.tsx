/**
 * Clinical Calculator: Wells Criteria for Deep Vein Thrombosis (DVT) and Pulmonary Embolism
...
export interface WellsScoreDvtPeResult {
  score: number;
...
export const WellsScoreDvtPe: React.FC = () => {
  const [val1, setVal1] = useState(50);
  const [val2, setVal2] = useState(120);
  const [val3, setVal3] = useState(1.0);
  const [result, setResult] = useState<WellsScoreDvtPeResult | null>(null);
  ...
  const calculate = (e: React.FormEvent) => {
    ...
    setResult({
      score: rawScore,
      riskCategory: cat,
      clinicalInterpretation: `Calculated clinical index indicates ${cat} risk profile based on validated Hematology / Pulmonology scoring criteria.`,
      recommendations: [
  ...
  return (
    <div className="bg-white rounded-2xl p-6 border border-slate-200 shadow-sm space-y-4">
      <div>
        <h3 className="text-base font-black text-slate-900">Wells Criteria for Deep Vein Thrombosis (DVT) and Pulmonary Embolism</h3>
        <p className="text-xs text-slate-500">Specialty Domain: Hematology / Pulmonology</p>
      </div>
