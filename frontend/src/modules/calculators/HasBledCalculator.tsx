/**
 * Clinical Calculator: HAS-BLED Score for Major Bleeding Risk on Anticoagulation
...
export interface HasBledCalculatorResult {
  score: number;
...
export const HasBledCalculator: React.FC = () => {
  const [val1, setVal1] = useState(50);
  const [val2, setVal2] = useState(120);
  const [val3, setVal3] = useState(1.0);
  const [result, setResult] = useState<HasBledCalculatorResult | null>(null);
  ...
  const calculate = (e: React.FormEvent) => {
    ...
    setResult({
      score: rawScore,
      riskCategory: cat,
      clinicalInterpretation: `Calculated clinical index indicates ${cat} risk profile based on validated Cardiology / Hematology scoring criteria.`,
      recommendations: [
  ...
  return (
    <div className="bg-white rounded-2xl p-6 border border-slate-200 shadow-sm space-y-4">
      <div>
        <h3 className="text-base font-black text-slate-900">HAS-BLED Score for Major Bleeding Risk on Anticoagulation</h3>
        <p className="text-xs text-slate-500">Specialty Domain: Cardiology / Hematology</p>
      </div>
