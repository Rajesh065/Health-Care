...
export const AppointmentDeskPage: React.FC<{
  ...
  return (
    <div className="space-y-6 py-2 w-full font-sans">
      ...
      <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-xs flex flex-col md:flex-row justify-between items-start md:items-center gap-4 w-full">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-0.5">
            Manage booked patient visits, dispatch tokens to specialist chambers, and assign inpatient ward beds.
          </p>
        </div>
      ...
      {activeSubTab === 'bed-allocation' ? (
        /* Inpatient Bed Allocation Matrix View */
        <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-xs space-y-4">
          <div className="flex justify-between items-center border-b border-slate-100 pb-3">
            <div>
              <h3 className="font-bold text-sm text-slate-900">Hospital Ward & Bed Allocation Matrix (450 Total Beds)</h3>
              <p className="text-xs text-slate-500">Live bed availability across Intensive Care, Cardiac, and Semi-Private Wings</p>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            <div className="p-4 bg-slate-50 border border-slate-200 rounded-xl space-y-2 text-xs">
              <span className="font-bold text-slate-900 block">ICU North Wing</span>
              <div className="text-2xl font-black text-rose-950">28 / 32</div>
              <span className="text-[10px] text-rose-700 font-bold bg-rose-50 border border-rose-200 px-2 py-0.5 rounded inline-block">
                4 Critical Beds Free
              </span>
            </div>

            <div className="p-4 bg-slate-50 border border-slate-200 rounded-xl space-y-2 text-xs">
              <span className="font-bold text-slate-900 block">Cardiac Step-Down Ward</span>
              <div className="text-2xl font-black text-slate-900">46 / 50</div>
              <span className="text-[10px] text-emerald-700 font-bold bg-emerald-50 border border-emerald-200 px-2 py-0.5 rounded inline-block">
                4 Telemetry Beds Free
              </span>
            </div>

            <div className="p-4 bg-slate-50 border border-slate-200 rounded-xl space-y-2 text-xs">
              <span className="font-bold text-slate-900 block">Post-Operative Orthopedics</span>
              <div className="text-2xl font-black text-slate-900">60 / 75</div>
              <span className="text-[10px] text-emerald-700 font-bold bg-emerald-50 border border-emerald-200 px-2 py-0.5 rounded inline-block">
                15 Recovery Beds Free
              </span>
            </div>

            <div className="p-4 bg-slate-50 border border-slate-200 rounded-xl space-y-2 text-xs">
              <span className="font-bold text-slate-900 block">General Medicine & Day Care</span>
              <div className="text-2xl font-black text-slate-900">260 / 293</div>
              <span className="text-[10px] text-emerald-700 font-bold bg-emerald-50 border border-emerald-200 px-2 py-0.5 rounded inline-block">
                33 Wards Free
              </span>
            </div>
              ...
              <div className="p-3.5 bg-slate-50 border border-slate-200 rounded-xl space-y-2">
                ...
                <div className="grid grid-cols-2 gap-2">
                  <button
                    ...
                    className="p-2 bg-white hover:bg-slate-100 border border-slate-200 rounded-lg text-center font-bold text-[11px] text-slate-800 cursor-pointer"
                  >
                    ✓ Mark Checked-In
                  </button>
                  <button
                    onClick={() => handleStatusChange(selectedApp.id, 'IN_CONSULTATION')}
                    className="p-2 bg-teal-800 hover:bg-teal-900 text-white rounded-lg text-center font-bold text-[11px] cursor-pointer"
                  >
                    ➔ Send to Chamber
                  </button>
                </div>
                <button
                  ...
                  className="w-full p-2 bg-emerald-700 hover:bg-emerald-800 text-white rounded-lg text-center font-bold text-[11px] cursor-pointer"
                >
